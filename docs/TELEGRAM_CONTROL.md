...
- 자동 현재후보 알림: VM은 `refine-entry-notify`를 주기적으로 실행해 정제된 과거 생존 전략이 현재 닫힌 봉 조건과 맞는 `READY` 후보로 바뀔 때 알려줍니다. `READY`가 없더라도 손익비/PF 기준은 통과했지만 현재 feature만 아직 안 맞는 관찰 후보를 기본 6시간마다 요약합니다. 둘 다 주문을 넣지 않습니다.
