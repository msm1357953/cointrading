...
- 2026-05-02: A current-market gate was added for refined strategies. `refine-entry-check` reads `data/strategy_refine_latest.json`, fetches recent live klines, compares the latest closed-bar features with each SURVIVED candidate condition, writes `data/refined_entry_latest.json`, and exposes Telegram `현재후보`/`진입후보` plus dashboard `현재장 진입후보`. `refine-entry-notify` can send Telegram only when a current READY candidate appears or changes. This still does not place orders; live-supervisor, preflight, live flags, and one-shot guards remain separate.
- 2026-05-02: Automatic Telegram reporting was cleaned up. The VM default now keeps only actionable current-entry alerts (`refine-entry-notify`) and final live-supervisor candidate alerts (`live-supervisor-notify`) enabled. Legacy strategy candidate summaries, paper trade-event spam, LLM risk summaries, vibe probes, and meta research summaries stay manual by default so Telegram does not mix past/backtest research with current tradable timing.
- 2026-05-02: Entry policy was narrowed to high payoff candidates. `refine-entry-check` now promotes a candidate to `READY` only if it clears risk/reward quality gates: minimum test count, average bps, full-sample average, profit factor, payoff ratio, TP/SL ratio, win-rate edge over breakeven, and positive walk-forward window ratio. `live-supervisor` now requires a fresh refined `READY` match before treating old signal-grid approval as actionable. Broad scalping/strategy paper-entry timers are disabled by default so weak wide-entry experiments stop producing misleading loss cycles.
- 2026-05-02: Refined-entry Telegram alerts now include a low-noise watchlist. `READY` still sends immediately on change, while payoff-qualified `WAIT` candidates are summarized periodically, default 6 hours, so it is visible which good historical setups are waiting for current-market feature alignment. Watchlist alerts are explicitly not entry signals.
- 2026-05-03: Refined-entry waitlist wording and gating were tightened. A payoff-qualified candidate is now `WAIT` only when the current market is close to activation with at most two near misses; distant historical patterns are marked `OBSERVE`/`관찰보류` and excluded from Telegram periodic watch alerts. This prevents cases like BTCUSDC `breakout_short` appearing as if the bot expects an immediate short while the current trend, Bollinger position, volume, and breakout flag are far from the rule.
- 2026-05-03: A tactical radar layer was added for the gap between "no live entry" and "nothing to do." It classifies current 15m/1h conditions into `READY`, `NEAR`, `WATCH`, or `AVOID` for playbook scenarios such as upside impulse wait-for-pullback, pullback long, pullback short, and failed-breakout short. Telegram `레이더`/`기회` and dashboard `레이더` show the current tactical step, while `cointrading-tactical-radar-notify.timer` sends low-noise alerts. This layer never places orders and does not override live-supervisor/preflight/live flags.
- 2026-05-04: A tactical paper bridge was added because live entry permission was structurally too hard to reach from radar-only candidates. `tactical-paper-step` manages existing tactical paper cycles and starts a dry-run `strategy_cycles` paper position only when tactical radar is `READY`; `NEAR`/`WATCH` still do not create entries. The VM deploy includes `cointrading-tactical-paper.timer` so current-market tactical candidates can build forward paper evidence before any live promotion discussion.
- 2026-05-04: A one-shot tactical live bridge was added for the first real-money path. `tactical-live-step --armed` uses actual Binance orders only when live env flags are enabled, tactical radar is `READY`, runtime risk allows entries, no actual exchange order/position exists, exchange filters pass, and the live one-shot guard is unused. The deploy unit sets an 80 USDC cap and consumes the one-shot after the first live cycle starts; tactical paper and tactical live cycles are managed separately so the paper timer cannot mutate live rows.
...
2. Feed tactical paper/live results back into live-supervisor/readiness reporting so `READY -> paper result -> live result` is visible instead of mixing radar wording with final live permission.
3. After any live one-shot, inspect exchange fills, reduce-only exit handling, realized fees, and Telegram event wording before resetting the one-shot for another real order.
4. Run heavy `strategy-mine`/`strategy-refine` locally, copy only the JSON results to the VM, then watch Telegram `발굴`/`정제`/`현재후보` and dashboard `메타전략`.
