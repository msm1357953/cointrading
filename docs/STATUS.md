...
2. Add a paper/live promotion bridge for current-market READY refined candidates before any live test. Historical survival and current-bar matching are still not enough by themselves.
3. Keep live order flags off until at least one candidate is refined `READY` and a refined-only forward paper path confirms it on current market data.
