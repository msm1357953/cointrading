...
2. Feed tactical paper results back into live-supervisor/readiness reporting so `READY -> paper result -> live discussion` is visible instead of mixing radar wording with final live permission.
3. Keep live order flags off until at least one candidate is current-market `READY` and forward paper confirms it on current market data.
