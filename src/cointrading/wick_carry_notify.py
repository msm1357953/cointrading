"""Run a wick lifecycle step and notify Telegram on key events.

Mirror of funding_carry_notify.py for the wick_long strategy.
"""
...
def default_state_path() -> Path:
    return default_db_path().parent / "wick_carry_notify_state.json"
...
def _format_open(opened: dict[str, Any], cfg: TradingConfig) -> str:
    ...
    wick = float(opened["wick_ratio"])
    drop = float(opened["drop_pct"])
    px = float(opened["entry_price"])
    stop_px = px * (1 - cfg.wick_carry_stop_loss_bps / 10_000.0)
    hold_h = cfg.wick_carry_max_hold_seconds / 3600
    return (
        "📈 wick OPEN\n"
        f"  {symbol} long @ {px:.6f}\n"
        f"  wick ratio {wick:.2f}, intrabar drop {drop * 100:.2f}%\n"
        f"  hold {hold_h:g}h, stop {stop_px:.6f} (-{cfg.wick_carry_stop_loss_bps:.0f} bps)\n"
        f"  notional {cfg.wick_carry_notional:.0f} USDC (paper)"
    )
...
def evaluate_live_ready(storage: TradingStore) -> LiveReadyStatus:
    ...
    return LiveReadyStatus(
        ready=not reasons, closed_n=closed_n,
        win_n=len(wins), loss_n=len(losses),
        sum_pnl=sum_pnl, win_rate=win_rate, reasons=reasons,
    )


def _format_live_ready(status: LiveReadyStatus) -> str:
    return (
        "🎯 wick: 라이브 검토 가능 시점 도달\n"
        ...
        "  3. .env: COINTRADING_WICK_CARRY_LIVE_ENABLED=true\n"
        "  → sudo systemctl restart cointrading-wick-engine.timer"
    )
...
def run_step_and_notify(*, state_path: Path | None = None) -> dict[str, Any]:
    ...
    if tcfg.bot_token and tcfg.default_chat_id:
        try:
            ...
            logger.warning("wick_carry_notify: TelegramClient init failed: %s", exc)
    ...
    if tclient is not None:
        for opened in result.opened:
            try:
                ...
                logger.warning("wick_carry_notify: send open failed: %s", exc)
        ...
        for managed in result.managed:
            ...
            try:
                ...
                logger.warning("wick_carry_notify: send close failed: %s", exc)
    ...
    if status.ready and not alert_active and tclient is not None:
        try:
            ...
            logger.warning("wick_carry_notify: send live-ready failed: %s", exc)
...
def main(argv: list[str] | None = None) -> None:
    p = argparse.ArgumentParser(description="wick step + telegram notifications")
